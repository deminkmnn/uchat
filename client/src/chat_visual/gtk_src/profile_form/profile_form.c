...
void on_deactivate_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    // Validate GTK_data and its components
    if (!GTK_data || !GTK_data->profile_data || !GTK_data->call_data || !GTK_data->call_data->ssl) {
        g_print("Error: Invalid GTK_data structure.\n");
        return;
    }



    // Validate delete_entry widget
    if (!GTK_IS_ENTRY(GTK_data->profile_data->delete_entry)) {
        g_print("Error: Invalid delete_entry widget.\n");
        return;
    }

    // Get the login from the delete entry field
    ...
    if (!user_login || strlen(user_login) == 0) {
        ...
        gtk_label_set_text(GTK_LABEL(GTK_data->profile_data->success_label), ""); // Clear success label
        return;
    }
    ...
    gtk_label_set_text(GTK_LABEL(GTK_data->profile_data->error_label), ""); // Clear error label
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    // Replace the minimal activate section with:
    ...
    GtkWidget *activate_button = gtk_button_new_with_label("Activate User");
    ...
    gtk_widget_add_css_class(deactivated_list, "login-list");
