...
void on_next_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    // Ensure the stack is initialized and has valid children
    if (!stack || current_index < 0) {
        g_warning("Invalid GtkStack or index.");
        return;
    }
    ...
    // Debugging message
    g_message("Switched to: %s", stack_children[current_index]);
}
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    GtkWidget *activate_label = gtk_label_new("Activate User");
