...
static void on_key_pressed(GtkEventControllerKey *controller, guint keyval, guint keycode, GdkModifierType state, gpointer user_data) {
    ...
    GtkEntry *login_entry = GTK_ENTRY(user_data);
    GtkListBox *login_list = GTK_LIST_BOX(g_object_get_data(G_OBJECT(controller), "list-box"));

    if (keyval == GDK_KEY_Tab) {
        const char *current_text = gtk_editable_get_text(GTK_EDITABLE(login_entry));
        const char *suggested_email = find_similar_email(current_text, login_list);

        if (suggested_email) {
            gtk_editable_set_text(GTK_EDITABLE(login_entry), suggested_email);
        }
    }
}

// Modified `on_login_row_selected` callback to update the delete_entry box
static void on_login_row_selected(GtkListBox *list_box, GtkListBoxRow *row, gpointer user_data) {
    ...
    if (row) {
        ...
        if (GTK_IS_WIDGET(child)) {
            ...
            if (GTK_IS_LABEL(box_child)) {
                const char *login_text = gtk_label_get_text(GTK_LABEL(box_child));

                // Set the delete_entry text to the selected login
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    GTK_data->profile_data->login_list = login_list;
    // Connect signal for row selection
    g_signal_connect(login_list, "row-selected", G_CALLBACK(on_login_row_selected), login_entry);
    ...
    g_signal_connect(key_controller, "key-pressed", G_CALLBACK(on_key_pressed), login_entry);
    gtk_widget_add_controller(login_entry, GTK_EVENT_CONTROLLER(key_controller));
