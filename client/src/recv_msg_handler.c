            ...
            if (command_code_json) {
                ...
                if(!stop_flag && counter == number_of_users){
                    ...
                    case 17:
                            ...
                            if (GTK_data->profile_data && GTK_data->profile_data->users_list) {
                                GtkWidget *child = gtk_widget_get_first_child(GTK_WIDGET(GTK_data->profile_data->users_list));
                                while (child) {
                                    GtkWidget *next = gtk_widget_get_next_sibling(child);
                            ...
                            for (int i = 0; i < number_of_users; i++) {
                                ...
                                if (!is_admin && GTK_data->profile_data && GTK_data->profile_data->admin_login_list) {
