...
// Main thread callback to update the login list
static gboolean update_login_list_main_thread(gpointer user_data) {
    profile_data_t *data = (profile_data_t *)user_data;
    if (data && data->login_list && data->parsed_json) {
        update_login_list(data->login_list, data->parsed_json);
        cJSON_Delete(data->parsed_json);
        g_free(data);
    }
    return G_SOURCE_REMOVE; // Remove the source after executing
}
            ...
            if(command_code_json) {
                ...
                if (stop_flag) {
                    if (command_code_json->valueint == 17) {
                        ...
                        profile_data_t *update_data = g_malloc(sizeof(profile_data_t));
                        update_data->login_list = GTK_data->profile_data->login_list;
                        update_data->parsed_json = parsed_json; // Pass ownership of parsed_json

                        g_main_context_invoke(NULL, update_login_list_main_thread, update_data);
                        } else {
